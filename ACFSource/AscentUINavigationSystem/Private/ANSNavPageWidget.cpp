...
void UANSNavPageWidget::GoToWidget(TSubclassOf<UUserWidget> nextPage)
{
    if (UISubsystem) {
        ...
        UISubsystem->SpawnInGameWidget(nextPage, false);
    }
}
...
void UANSNavPageWidget::NativeConstruct()
{
    ...
    SetStartFocus(Cast<UANSNavWidget>(GetDesiredFocusTarget()));
...
void UANSNavPageWidget::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
    ...
    if (controller) {
        ...
        } else if (GetInputSubsystem()->GetCurrentInputType() == ECommonInputType::MouseAndKeyboard) {
...
void UANSNavPageWidget::Internal_SetFocusToWidget(const FFocusedWidget& widget, const FGeometry& MyGeometry)
{
    ...
    if (widget.focusedWidget) {
        ...
        /*
        TSharedPtr<SWidget> widgeptr = widget.focusedWidget->GetCachedWidget();
        if (widgeptr.IsValid()) {
             widgeptr->OnMouseEnter(MyGeometry, FPointerEvent());

            TSharedPtr<SPanel> subChild = StaticCastSharedPtr<SPanel>(widgeptr);
            if (subChild.IsValid()) {
                FChildren* widgetRef = subChild->GetChildren();
                if (widgetRef) {
                    for (int32 i = 0; i < widgetRef->Num(); i++) {
                        TSharedRef<SWidget> innerWid = widgetRef->GetChildAt(i);
                       innerWid->OnMouseEnter(MyGeometry, FPointerEvent());
                    }
                }
            }
        }*/
...
void UANSNavPageWidget::Internal_SetStartFocus()
{
    if (startFocusedWidget.focusedWidget) {
        ...
        if (widgetHanlde) {
            ...
            //    FWidgetPath NewFocusedWidgetPath = WidgetsUnderCursor.GetPathDownTo(widgetHanlde.ToSharedRef());
            //   FSlateApplication::Get().SetUserFocus(FSlateApplication::Get().GetUserIndexForController(0), NewFocusedWidgetPath, EFocusCause::Mouse);
...
{
    if (focusedWidget.focusedWidget->IsValidLowLevelFast()) {
        ...
        /* TSharedPtr<SWidget> widgeptr = focusedWidget.focusedWidget->GetCachedWidget();
        if (widgeptr.IsValid()) {
            widgeptr->OnMouseLeave(FPointerEvent());
        }
        ...
        if (subChild.IsValid()) {
            ...
            if (widgetRef) {
                for (int32 i = 0; i < widgetRef->Num(); i++) {
                    TSharedRef<SWidget> innerWid = widgetRef->GetChildAt(i);
                    innerWid->OnMouseLeave(FPointerEvent());
                }
            }
        }*/
        if (focusedWidget.parentNavWidget->IsValidLowLevelFast(true)) {
