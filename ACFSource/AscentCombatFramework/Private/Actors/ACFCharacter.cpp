...
void AACFCharacter::InitializeCharacter()
{
    if (HasAuthority() && !bInitialized)
    {
        ...
        if (EquipmentComp)
        {
            EquipmentComp->OnEquippedArmorChanged.AddDynamic(this, &AACFCharacter::HandleArmorChanged);
...
void AACFCharacter::AssignTeam(ETeam team)
{
    ...
    SetGenericTeamId(static_cast<uint8>(CombatTeam));
...
void AACFCharacter::HandleArmorChanged(const FGameplayTag& itemSot)
{
    DamageHandlerComp->InitializeDamageCollisions(GetCombatTeam());
}
...
void AACFCharacter::TriggerAction(FGameplayTag Action, EActionPriority Priority, bool canBeStored /*= false*/, const FString& contextString /*= ""*/, AActor* InteractedActor, FGameplayTag ItemSlotTag)
{
    if (ActionsComp)
    {
        ActionsComp->TriggerAction(Action, Priority, canBeStored, contextString, InteractedActor, ItemSlotTag);
        return;
    }
...
float AACFCharacter::TakeDamage(float Damage, const FDamageEvent& DamageEvent,
...
float AACFCharacter::GetBaseDamage_Implementation()
{
    return 1.f;
}
...
void AACFCharacter::SetGenericTeamId(const FGenericTeamId& InTeamID)
{
    ...
    CombatTeam = static_cast<ETeam>(InTeamID.GetId());

}

FGenericTeamId AACFCharacter::GetGenericTeamId() const
{
    return static_cast<uint8>(CombatTeam);
}
