...
public:
    ...
    UFUNCTION(BlueprintPure, Category = ACF)
    FORCEINLINE TSubclassOf<UGameplayEffect> GetUnsheatedGameplayEffect() const
    {
        return GameplayModifier;
    }

    UFUNCTION(BlueprintCallable, Category = ACF)
    void SetUnsheatedGameplayEffect(const TSubclassOf<UGameplayEffect>& inGameplayModifier)
    {
    ...
    UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "ACF", meta = (ExposeOnSpawn = "true"))
    bool bResourceTool;
...
protected:
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "ACF | Weapon|ARS")
    FAttributesSetModifier UnsheathedAttributeModifier;

    /*Gameplay Effect applied once this item is equipped in hands*/
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "ACF | Weapon|GAS")
