...
UNomadHazardDoTStatusEffect* UNomadStatusEffectManagerComponent::ApplyHazardDoTEffectWithPercent(const TSubclassOf<UNomadBaseStatusEffect>& EffectClass, float DotPercent)
{
    if (!EffectClass)
        return nullptr;
    UNomadHazardDoTStatusEffect* Effect = NewObject<UNomadHazardDoTStatusEffect>(GetOwner(), EffectClass);
    if (Effect)
    {
